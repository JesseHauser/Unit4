...
def count_q_tips(list_choice):
    ...
    for list in shopping_lists:
        for item in list:
            if item == 'q-tips':
                count += 1
                print(f"there are {count} qtips in this list")

# In order to make the shopping lists more calcium rich, write a function, drink_more_milk, that adds 'milk' to each 
# of the lists (unless it's already there).        
def drink_more_milk():
    for i in range(len(shopping_lists)):
        if 'milk' not in shopping_lists:
            shopping_lists[i].append('milk')
            

# Write a function if_you_give_a_moose_a_cookie, that will go through every 
# element of shopping_cart and update 'milk' to be 'milk and cookies'
...
while True:
    user_input =input("what would you like to do, Choose 1 = update item, 2 = view item, 3 = view list, 4 = combine all list, 5 = count q tips in list, 6 = add milk in list not containg milk, 7 = change all milk in list to milk and cookies or q = quit: ")
    if user_input == '1':
        ...
        user_input = int(input("Which list do you want to know the number of qtips in? ")) -1

        count_q_tips(user_input)
